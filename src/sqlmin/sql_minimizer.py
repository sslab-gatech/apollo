...
#FIXME: support both library and process mode for sqlite
#FIXME: remove many duplicated code
#FIXME: recover the previous routine when we extract subquery
...
def contain_limit(line):
    if 'limit' in line.lower() and any(i.isdigit() for i in line) and ' ' in line:
        return True
    return False
...
def split_block_by_indent(query):
    ...
    for process_indent in num_kinds:
        ...
        for index, line in enumerate(query.split("\n")):
            ...
            if current_indent == process_indent and open_flag == False:
                start = index
                open_flag = True                    
                continue
...
class Minimizer(object):
    def __init__ (self, filename, dbname, outdir, cleanlog=True):
        ...
        if dbname == 'sqlite':
            ...
            self.ori_query_time_old, self.ori_query_time_new = self.run_queries_sqlite(self.org_query)
            print "Old_time:", self.ori_query_time_old, "New_time", self.ori_query_time_new, "Ratio", self.ori_query_time_new/self.ori_query_time_old
            ...
            if self.ori_query_time_new < self.ori_query_time_old:
                ...
                print "[!] Old version takes loger time, this is not regression"
                self.good_to_go = False

            # discard too short execution
            if self.ori_query_time_old < 0.001:
                self.good_to_go = False
            self.timeout = calc_timeout(self.ori_query_time_new, self.ori_query_time_old)
            mkdirs(TMP_QUERY_STORE)

        elif dbname == 'postgres':
            self.ori_query_time_new, self.ori_query_time_old = self.run_original()              
            print "Old_time:", self.ori_query_time_old, "New_time", self.ori_query_time_new
    ...
    def run_query_latest(self, query):
        ...
        if self.dbname == 'sqlite':
            ...
            elapsed, err, out = self.run_query(query, LATEST_VERSION_PORT, timeout=str(2))
    ...
    def run_query(self, query, port, timeout=None):
        ...
        elapsed, err, out = run_cmd (cmd, str(port), no_stdout=False, timeout=timeout)
        return elapsed, err, out
    ...
    def run_original(self):
        ...
        elapsed, err, out = self.run_query(query, LATEST_VERSION_PORT, timeout="7")        
        elapsed2, err2, out2 = self.run_query(query, OLDEST_VERSION_PORT, timeout="7")        
    ...
    def reduction_subquery(self, threshold):
        ...
        query_parsed = sqlparse.parse(start_query, query_reduction=reduction_param)[0]
        tokens = query_parsed.tokens 
        ...
        for filename in tqdm(filelist):
            ...
            with open(filename, 'r') as f:
                ...
                #print "  >>>", filename, actual_time, len(temp_query), actual_time>self.timeout
        ...
        print " > Query size is reduced %d ===> %d" % (len(start_query), os.path.getsize(pn))
        
        #elapsed, err, out = self.run_query(temp_query, LATEST_VERSION_PORT, timeout=str(0.5))
        #actual_time = retrieve_actual_time(out)
        actual_time = self.run_query_latest(temp_query)

        if actual_time > self.timeout:
            print " > Elapsed time %6.3f (longer than TIMEOUT)" % actual_time
        else:            
            #raise Exception('Generated query is wrong !!!!!')
    ...
    def reduction_select_from_element(self):
        ...
        if self.current_query == "":
            ...
            #print "use current query"
            fixed_query = fix_as_statement(self.current_query)
            start_query = self.current_query

        elements = elements_between_select_from(fixed_query)
        replace_candidate = replace_candidate_between_select_from(fixed_query, elements)
        ...
        for candidate, replaced_str in tqdm(replace_candidate):
            ...
            #elapsed, err, out = self.run_query(temp_query, LATEST_VERSION_PORT, timeout=str(self.timeout))
            #elapsed, err, out = self.run_query(temp_query, LATEST_VERSION_PORT, timeout=str(0.5))
            #actual_time = retrieve_actual_time(out) 
            actual_time = self.run_query_latest(temp_query)
            #print "  >>>", filename, actual_time, len(temp_query), actual_time>self
        ...
        for candidate, replaced_str in replace_candidates:            
            intermediate_query = intermediate_query.replace(candidate, replaced_str)
        ...
        for line in final_before_limit.split("\n"):
            if contain_limit(line):
                ...
                #matched_str.append(re.match('limit.[0-9]+', line).group())
    ...
    def reduction_using_parenthesis(self):
        ...
        for stack, item in tqdm(list(parenthetic_contents(start_query))):
            ...
            actual_time = self.run_query_latest(temp_query)        
            #print "  >>>", actual_time, len(temp_query), actual_time>self.timeout
            if os.environ.has_key("DEBUG"):
                print_log([elapsed, temp_query])
        ...
        if actual_time > self.timeout:            
            print " > Elapsed time %6.3f (longer than TIMEOUT)\n" % actual_time
        else:                        
            print " > [WARNING] This strategy failed to reduce query size!!!"        
    ...
    def reduction_by_indent(self):
        ...
        for _str in tqdm(matched_list):
            ...
            #print "  >>>", actual_time, len(reindent_query), actual_time>self.timeout
    ...
    def storefile(self, target_dir, filename):        
        ...
        print "[*] Final result: actual_time = %f, timeout = %f" % (actual_time, self.timeout)
...
class Report(object):
    ...
    def add_entry(self, filename, org_size, reduced_size, elapsed_time, org_time=None, reduced_time=None):
        reduced_ratio = str("%.4f" % float(float(reduced_size) / org_size))
        self.storage[filename] = [filename, org_size, reduced_size, reduced_ratio, elapsed_time]
...
if __name__ == "__main__":
    ...
    # TODO: clean up several variables
    TMP_ERR  = os.path.join(args.outdir, ".stderr")
    TMP_ERR_PN   = os.path.join(args.outdir, ".minimizer_err")
    TMP_QUERY_PN = os.path.join(args.outdir, ".minimizer_query")
    LOGFILE  = "qmin_log"
    ...
    for filename in filelist:
        ...
        start = time.time()
        mini = Minimizer(filename, args.dbname, args.outdir)
        mini.init_test()

        if mini.good_to_go == False:
            continue
                    
        mini.bottomup(15)
        mini.topdown(15)
        
        mini.storefile(args.outdir, filename) 
        elapsed_time = time.time() - start
        report.add_entry(filename, mini.org_size, len(mini.current_query), elapsed_time)
        report.print_report()