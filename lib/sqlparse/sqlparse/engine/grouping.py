...
def group(stmt, query_reduction=None):    
    # stmt is already tokenized but not grouped
    global reduction
...
def process_reduction(tlist, reduction_param):
    global counter 

    org_query       = reduction_param['org_query']
    token_threshold = reduction_param['token_threshold']
    work_dir        = reduction_param['work_dir']
    
    if len(list(tlist)) > token_threshold:
        
        with open("%s/query%d" % (work_dir, counter), 'w') as f: 
            current_token = str(tlist.value).lstrip()
            current_token2 = " ".join(current_token.split())            
            
            if is_select_statement(current_token2): 
                current_query = org_query.replace(current_token, "(select 1)")                

                if os.environ.has_key("DEBUG"):                    
                    print_log([current_query, current_token])   

            elif is_case_statement(current_token):
                current_query = org_query.replace(current_token, "")
            
            # TODO: find another cases
            else:
                current_query = ""

            if not current_query.endswith(';'):
                current_query = current_query + "\n;"

                f2 = open("%s/query%d_token" % (work_dir, counter), 'w')
                f2.write(current_token)
                f2.close()
...
def _group(tlist, cls, match,
    ...
    # default Token    : is_group=False
    # default TokenList: is_group=True
    # post()           : specified by each handling function
    ...
    # if parameter is passed from sql_minimizer
    if reduction != None:        
        process_reduction(tlist, reduction)
