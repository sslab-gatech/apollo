...
int main(int argc, char *argv[])
{
  ...
  regex optregex("--(help|log-to|verbose|target|sqlite|monetdb|version|dump-all-graphs|dump-all-queries|seed|dry-run|max-queries|rng-state|prob-table|exclude-catalog)(?:=((?:.|\n)*))?");
  ...
  if (options.count("help")) {
    cerr <<
      ...
      "    --prob-table=string  probability table path" << endl <<
  ...
  try
	...
	schema = make_shared<schema_pqxx>(options["target"], options.count("exclude-catalog"));
      ...
      if (options.count("prob-table")) {
      	cout << "[*] User-requested probability table" << endl;
        std::fstream probfile(options["prob-table"], std::ios_base::in);        
