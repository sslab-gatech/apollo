...
extern int probability[15];

//JJ: probability table
shared_ptr<table_ref> table_ref::factory(prod *p) {
  try {
    if (p->level < 3 + d6()) {
      if (d1000() < probability[9] && p->level < d6())
	      return make_shared<table_subquery>(p);
      if (d1000() < probability[10])
...
table_sample::table_sample(prod *p) : table_ref(p) {
  ...
  percent = 0.1 * d100();
  method = (d1000() < 332) ? "system" : "bernoulli";
}
...
joined_table::joined_table(prod *p) : table_ref(p) {
  ...
  if (d1000() < probability[11]) {
    type = "inner";
  } else if (d1000() < probability[12]) {
    type = "left";
  } else if (d1000() < probability[13]){
    type = "right";
  } else {
    type = " "; //default join
  }
  ...
  prod(p), myscope(s)
{
  ...
  //JJ: probability table (distinct)
  set_quantifier = (d1000() < probability[3]) ? "distinct" : "";

  search = bool_expr::factory(this);

  //JJ: probability table (limit)
  if (d1000() < probability[4]) {
    ostringstream cons;
...
//JJ: we are not making a probability table here 
...
//JJ: probability table
// ==> we don't need this, we don't update/insert
