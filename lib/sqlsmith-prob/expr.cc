    ...
    : value_expr(p), expr("")
{
  ...
  if (type == scope->schema->inttype)
    ...
    expr += (d1000() < 500) ? scope->schema->true_literal : scope->schema->false_literal;
  else if (dynamic_cast<insert_stmt*>(p) && (d1000() < 500))
    expr += "default";
  else
